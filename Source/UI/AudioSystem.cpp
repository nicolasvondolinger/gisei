...
SoundHandle AudioSystem::PlaySound(const std::string& soundName, bool looping)
{
    ...
    int channel = Mix_PlayChannel(-1, sound, looping ? -1 : 0);

    if (channel == -1)
    {
        SDL_Log("[AudioSystem] Failed to play sound %s: No free channels available.", soundName.c_str());
        return SoundHandle::Invalid;
    }
    ...
    // Armazena as informações sobre este som
    HandleInfo info;
    ...
    // Salva as informações de rastreamento
    mHandleMap.emplace(mLastHandle, info);
    mChannels[channel] = mLastHandle;
    
    // Retorna o handle para o chamador (ex: Ship, Game)
    return mLastHandle;
